...
static char *inner_cong_alg = "cubic";
module_param(inner_cong_alg, charp, 0644);
MODULE_PARM_DESC(inner_cong_alg, "Inner congestion control algorithm used by Spine");
...
static void do_set_cwnd(struct ccp_connection *conn, uint32_t cwnd)
{
	struct sock *sk;
	struct tcp_sock *tp;
...
static void do_set_rate_abs(struct ccp_connection *conn, uint32_t rate)
{
	struct sock *sk;
...
void tcp_spine_in_ack_event(struct sock *sk, u32 flags)
{
	...
	for (i = 0; i < MAX_SKB_STORED; i++) {
		if (skb) {
			...
			ca->skb_array[i].interval_us = tcp_stamp_us_delta(
...
EXPORT_SYMBOL_GPL(tcp_spine_in_ack_event);
...
void tcp_spine_cong_control(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (conn != NULL) {
		...
		if (ok == LIBCCP_FALLBACK_TIMED_OUT) {
			pr_info("libccp fallback timed out");
			// TODO default to cubic?
		}

		ca->conn->prims.was_timeout = false;
	} else {
		pr_info("ccp_connection not initialized");
	}
}
EXPORT_SYMBOL_GPL(tcp_spine_cong_control);

/* Slow start threshold is half the congestion window (min 2) */
u32 tcp_spine_ssthresh(struct sock *sk)
{
	const struct tcp_sock *tp = tcp_sk(sk);

	return max(tp->snd_cwnd >> 1U, 2U);
}
EXPORT_SYMBOL_GPL(tcp_spine_ssthresh);

u32 tcp_spine_undo_cwnd(struct sock *sk)
{
	const struct tcp_sock *tp = tcp_sk(sk);

	return max(tp->snd_cwnd, tp->snd_ssthresh << 1);
}
EXPORT_SYMBOL_GPL(tcp_spine_undo_cwnd);

void tcp_spine_pkts_acked(struct sock *sk, const struct ack_sample *sample)
{
	struct ccp *cpl;
	s32 sampleRTT;
...
EXPORT_SYMBOL_GPL(tcp_spine_pkts_acked);
 ...
 */
void tcp_spine_set_state(struct sock *sk, u8 new_state)
{
...
EXPORT_SYMBOL_GPL(tcp_spine_set_state);

void tcp_spine_init(struct sock *sk)
{
	struct spine *spine_ca;
	struct tcp_sock *tp = tcp_sk(sk);
	u32 inner_ca_key;
	// this won't be used later
	bool ca_ecn;
	...
	struct tcp_congestion_ops *inner_ops;

	pr_info("new flow\n");
	spine_ca = inet_csk_ca(sk);
	spine_ca->conn = spine_connection_start(kernel_datapath, (void *)sk, &dp_info);
	if (spine_ca->conn == NULL) {
		pr_info("start connection failed\n");
	} else {
		pr_info("starting connection %d", spine_ca->conn->index);
	}
	// init inner congestion control opertions
	inner_ca_key =
		tcp_ca_get_key_by_name(sock_net(sk), inner_cong_alg, &ca_ecn);
	if (inner_ca_key == TCP_CA_UNSPEC) {
		pr_info("Cannot find the tcp congestion control algorithm %s\n",
			inner_cong_alg);
		inner_ops == NULL;
	} else {
		// find and init inner ca
		inner_ops = tcp_ca_find_key(inner_ca_key);
		inner_ops->init();
	}
...
EXPORT_SYMBOL_GPL(tcp_spine_init);

void tcp_spine_release(struct sock *sk)
{
	struct ccp *cpl = inet_csk_ca(sk);
	if (cpl->conn != NULL) {
...
EXPORT_SYMBOL_GPL(tcp_spine_release);

struct tcp_congestion_ops tcp_spine_congestion_ops = {
	// .flags = TCP_CONG_NEEDS_ECN,
	.in_ack_event = tcp_spine_in_ack_event,
	.name = "spine",
	.owner = THIS_MODULE,
	.init = tcp_spine_init,
	.release = tcp_spine_release,
	.ssthresh = tcp_spine_ssthresh,
	//.cong_avoid = tcp_spine_cong_avoid,
	.cong_control = tcp_spine_cong_control,
	.undo_cwnd = tcp_spine_undo_cwnd,
	.set_state = tcp_spine_set_state,
	.pkts_acked = tcp_spine_pkts_acked
};

void spine_log(struct ccp_datapath *dp, enum spine_log_level level,
...
static int __init tcp_spine_register(void)
{
	int ok;
	...
	if (!kernel_datapath) {
		pr_info("could not allocate ccp_datapath\n");
		return -4;
	}
	...
	kernel_datapath->ccp_active_connections =
		(struct spine_connection *)kzalloc(sizeof(struct spine_connection) *
							 MAX_ACTIVE_FLOWS,
						 GFP_KERNEL);
	...
	ok = spine_init(kernel_datapath, 0);
	if (ok < 0) {
		pr_info("ccp_init failed: %d\n", ok);
		free_spine_nl_sk();
		return -6;
	}

	pr_info("init\n");
	return tcp_register_congestion_control(&tcp_spine_congestion_ops);
}

static void __exit tcp_spine_unregister(void)
{
	tcp_unregister_congestion_control(&tcp_spine_congestion_ops);
	free_spine_nl_sk();
...
module_init(tcp_spine_register);
module_exit(tcp_spine_unregister);
